...
class Solution:
    ...
    def simplify_path(self, path: str) -> str:
        ...
        list_of_paths = list(
            filter(lambda x: x != "", re.split("/", consecutive_slashes_removed))
        )
        final_paths = []
    ...
    def simplify_path_improved(self, path: str) -> str:
        single_dot_removed = re.sub(r"(?<=(/))\.(?=(/|$))", "", path)
        num_of_double_dot = len(
            re.findall(r"(?<=(/))\.\.(?=(/|$))", single_dot_removed)
        )
        double_dot_removed = single_dot_removed
        for _ in range(num_of_double_dot):
            double_dot_removed = re.sub(
                r"(?<=(/))([a-zA-Z0-9_]*[/]+)?\.\.(?=(/|$))", "", double_dot_removed
            )
