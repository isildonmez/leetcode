...
class Solution:
    ...
    def is_same_solution(self, p: Optional[TreeNode], q: Optional[TreeNode]) -> bool:
        if p is not None and q is not None:
            if p.val == q.val:
                return self.is_same_solution(p.left, q.left) and self.is_same_solution(p.right, q.right)
...
if __name__ == "__main__":
    x = TreeNode(1, TreeNode(2, TreeNode(4), TreeNode(5)), TreeNode(3, TreeNode(6), None))
    y = TreeNode(1, TreeNode(2, TreeNode(4), TreeNode(3)), TreeNode(3, TreeNode(6), None))
